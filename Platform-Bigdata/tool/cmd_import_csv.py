...
def import_random_csv(filename, older_cmd = None, sep=None, dbname="open_data_lake"):
	...
	if "ERROR" in res1:
		...
		for v in var:
			if v.strip('"') in res2 and f"psql -d {dbname} -c \" ALTER TABLE " + filename.split(".")[0] + " ALTER COLUMN " + v + " TYPE VARCHAR;\"" != older_cmd:
				...
				break
		if pb:
			return import_random_csv(filename, older_cmd, sep, dbname)
		else:
...
if __name__ == "__main__":
	...
	parser.add_argument('-d', type=str, help='Database name, default : open_data_lake')
	parser.add_argument('-f', type=str, help='Filename')
	parser.add_argument('-s', type=str, help='Separator needed just if the separator is not , or ;')

	args = parser.parse_args()

	if args.d is None:
		args.d = "open_data_lake"

	if args.f:
		import_random_csv(filename=args.f, older_cmd = None, sep=args.s, dbname=args.d)
